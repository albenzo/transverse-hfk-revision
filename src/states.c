...
LiftStateRBTreeNode_t NIL_LIFT_NODE = {
    BLACK, -1, NULL, &NIL_LIFT_NODE, &NIL_LIFT_NODE, &NIL_LIFT_NODE};
 ...
 * @param G a lift grid
 */
void copy_lift_state(LiftState *dest, const LiftState *const origin,
 ...
 */
int eq_lift_state(const LiftState a, const LiftState b,
                  const LiftGrid_t *const G) {
  for (int i = 0; i < G->sheets; ++i) {
 ...
 * @param v a lit state
 * @param G a lift grid
 * @return 0 if the states are equivalent, if u<v returns a negative number, and
 * positive if u>v
 */
int comp_lift_state(const LiftState u, const LiftState v,
                    const LiftGrid_t *const G) {
  for (int i = 0; i < G->sheets; ++i) {
...
void mirror_lift_state(LiftState *state, const LiftGrid_t *const G) {
  ...
  for (int i = 0; i < G->sheets; ++i) {
    ...
    for (int j = 0; j < G->arc_index - 1; ++j) {
      (*state)[i][j + 1] =
          original[G->sheets - (i + 1)][G->arc_index - (j + 1)];
    }
  }
 ...
 * @return index of a within b
 */
int get_lift_number(const LiftState a, const LiftStateList b,
                    const LiftGrid_t *const G) {
  LiftStateList temp;
  int count = 1;
  temp = b;
 ...
 */
LiftStateRBTree copy_lift_tree(LiftStateRBTree orig, LiftStateRBTree parent,
                               const LiftGrid_t *const G) {
 ...
 * @param G a lift grid
 */
void insert_data(LiftStateRBTree *root, LiftState s,
                 const LiftGrid_t *const G) {
 ...
 * @see insert_data
 */
void insert_tagged_data(LiftStateRBTree *root, LiftState s, int tag,
                        const LiftGrid_t *const G) {
  LiftStateRBTree node = malloc(sizeof(LiftStateRBTreeNode_t));
  node->data = s;
  node->tag = tag;
 ...
 * @param G a lift grid
 */
void insert_node(LiftStateRBTree *root, LiftStateRBTree z,
                 const LiftGrid_t *const G) {
 ...
 * @param G a lift grid
 */
void delete_data(LiftStateRBTree *root, LiftState s,
                 const LiftGrid_t *const G) {
 ...
 */
LiftStateRBTree find_node(const LiftStateRBTree *const root, LiftState s,
                          const LiftGrid_t *const G) {
  LiftStateRBTree iter = *root;
 ...
 */
int is_member(const LiftStateRBTree *const root, LiftState s,
              const LiftGrid_t *const G) {
  LiftStateRBTree node = find_node(root, s, G);
 ...
 */
int has_next(LiftTreeIter_t *iter) { return iter->cur_node != EMPTY_LIFT_TREE; }
 ...
 */
int is_empty(LiftTreeIter_t *iter) { return iter->cur_node == EMPTY_LIFT_TREE; }
...
StateRBTree copy_tree(StateRBTree orig, StateRBTree parent,
                      const Grid_t *const G) {
  if (EMPTY_TREE == orig) {
    return EMPTY_TREE;
  }
 ...
 * @see insert_data
 */
void s_insert_tagged_data(StateRBTree *root, State s, int tag,
                          const Grid_t *const G) {
  StateRBTree node = malloc(sizeof(StateRBTreeNode_t));
  node->data = s;
  node->tag = tag;
 ...
 * @return the node containing s, returns NULL if not found
 */
StateRBTree s_find_node(const StateRBTree *const root, State s,
                        const Grid_t *const G) {
  StateRBTree iter = *root;
 ...
 * @return if the iterator has another node
 */
int s_has_next(StateTreeIter_t *iter) { return iter->cur_node != EMPTY_TREE; }
 ...
 * @return if the iterator is empty
 */
int s_is_empty(StateTreeIter_t *iter) { return iter->cur_node == EMPTY_TREE; }
